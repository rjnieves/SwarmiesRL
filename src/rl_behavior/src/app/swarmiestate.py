...
class SwarmieState(object):
  ...
  def __init__(self, swarmie_name):
    ...
    self._position_storage = np.zeros([2, 2, 3])
    self._positions_populated = np.zeros([2, 2, 3], dtype=np.bool)
    self.linear_vel = None
    self.angular_vel = None

  def _set_position_storage(self, segment_addr, new_val):
    if len(segment_addr) != 2:
      raise RuntimeError('SwarmieState set operation invalid address.')
    if len(new_val) == 2:
